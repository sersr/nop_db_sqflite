...
// ensureInitialized();
  ...
  final myCacheTable = db.myCache;
  final indexsTable = db.indexs;
  test('database', () async {
    final i =
        myCacheTable.insert.insertTable(MyCache(name: 'MyCache', indexs: 9));
    print(i);
    i.go;
    myCacheTable.insert.insertTable(MyCache(name: 'MyCache2', indexs: 10)).go;
    ...
    indexsTable.insert
        ...
        .go; // q
    indexsTable.insert
        .insertTable(Indexs(chapterName: 'MyCache', indexs: 9))
        .go; // q MyCache => hello world2
    indexsTable.insert
    ...
    final query = myCacheTable.query;
    ...
    final q = myCacheTable.query;
    q.join(indexsTable)
      ..select.count.all
      ..using.myCache_indexs
    ...
    print(q.go);
    final qx = myCacheTable.query.myCache_name.join(indexsTable)
    ...
    myCacheTable.query.myCache_id.where
    ...
    myCacheTable.query.join(indexsTable)
    ...
    final using = myCacheTable.query.join(indexsTable)
    ...
    u.go;
    dashed;
    myCacheTable.query.join(indexsTable).where
    ...
    final t = myCacheTable.query.let((s) {
      s.where
    ...
    myCacheTable.query.all..let((s) => print('hahha: $s'));
    await Future.delayed(Duration.zero);
  });

  test('is null', () {
    myCacheTable.query.all
